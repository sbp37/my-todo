...
const SKINS=['#f0c8a0','#e8b088','#c98d63','#f5d4b8','#a97750'];
const HAIRCOL=['#3b2d24','#1d1b20','#6b4a2f','#8a5a3a','#4a4a58','#7a3a30','#a87838','#5a3a52'];
const BANDS=['#c95a4a','#3a5a8c','#4a8a6a','#c9a13a'];
function look(id){const h=hash(id);return{skin:SKINS[h%SKINS.length],hair:HAIRCOL[(h>>3)%HAIRCOL.length],hs:(h>>6)%6,glasses:h%7===0,band:BANDS[(h>>4)%BANDS.length]}}
...
/* ----- 아바타(작은 얼굴) 데이터URL 캐시 ----- */
const AV={};
function avatarURL(seed){if(AV[seed])return AV[seed];const h=hash(seed),lk=look(seed),cv=document.createElement('canvas');cv.width=cv.height=16;const c=cv.getContext('2d');
  px(c,0,0,16,16,PLATES[h%PLATES.length]);px(c,2,2,12,12,'#efe5cd');
  px(c,4,4,8,7,lk.skin);
  /* 헤어스타일 */
  if(lk.hs===0){px(c,3,2,10,3,lk.hair)}
  else if(lk.hs===1){px(c,3,2,10,3,lk.hair);px(c,2,4,3,7,lk.hair);px(c,11,4,3,7,lk.hair)}
  else if(lk.hs===2){px(c,3,2,10,3,lk.hair);px(c,6,0,4,3,lk.hair)}
  else if(lk.hs===3){px(c,2,2,10,3,lk.hair);px(c,9,4,4,2,lk.hair)}
  else if(lk.hs===4){px(c,3,2,10,3,lk.hair);px(c,3,3,10,1,lk.band)}
  else{px(c,3,2,10,2,lk.hair)}
  px(c,5,7,1,2,OUTLINE);px(c,10,7,1,2,OUTLINE);
  if(h%4===0)px(c,6,10,4,1,'#a05543');else px(c,6,10,4,1,OUTLINE);
  if(lk.glasses){px(c,4,6,3,3,'#3a3448');px(c,9,6,3,3,'#3a3448')}
...
function drawHair(c,x,yy,hs,hair,band){if(hs===0){px(c,x+2,yy,8,2,hair);px(c,x+1,yy+2,10,1,hair)}
  ...
  else if(hs===2){px(c,x+2,yy,8,2,hair);px(c,x+1,yy+2,10,1,hair);px(c,x+4,yy-2,4,2,hair)}
  else if(hs===3){px(c,x+1,yy,9,2,hair);px(c,x+1,yy+2,10,1,hair);px(c,x+8,yy+2,3,3,hair)}
  else if(hs===4){px(c,x+2,yy,8,2,hair);px(c,x+1,yy+2,10,1,hair);px(c,x+1,yy+2,10,1,band)}
  else{px(c,x+2,yy,8,1,hair);px(c,x+1,yy+1,10,1,hair)}}
function drawChar(c,x,y,o){const {hair,shirt,pose,frame,blink}=o,skin=o.skin||'#f0c8a0',hs=o.hs||0,band=o.band||'#c95a4a',gl=o.glasses;
  const bob=frame&&(pose==='type'||pose==='panic'||pose==='walk')?-1:0,yy=y+bob;
  if(pose==='stand'||pose==='walk'){drawHair(c,x,yy,hs,hair,band);px(c,x+2,yy+3,8,4,skin);if(!blink){px(c,x+4,yy+4,1,1,OUTLINE);px(c,x+7,yy+4,1,1,OUTLINE)}else{px(c,x+4,yy+4,2,1,OUTLINE);px(c,x+7,yy+4,2,1,OUTLINE)}if(gl){px(c,x+3,yy+3,3,3,'#3a3448');px(c,x+6,yy+3,3,3,'#3a3448')}px(c,x+2,yy+7,8,7,shirt);px(c,x+1,yy+8,1,5,skin);px(c,x+10,yy+8,1,5,skin);if(pose==='walk'&&frame){px(c,x+3,yy+14,3,6,OUTLINE);px(c,x+7,yy+14,3,5,'#3a3448')}else{px(c,x+3,yy+14,3,6,'#3a3448');px(c,x+7,yy+14,3,6,'#3a3448')}return}
  drawHair(c,x,yy,hs,hair,band);px(c,x+2,yy+3,8,4,skin);
  ...
  if(gl){px(c,x+4,yy+3,3,3,'#3a3448');px(c,x+7,yy+3,3,3,'#3a3448')}
...
function drawRoomCell(c,r,t){const state=roomState(t),h=hash(t.id),tk=animFrame(),frame=tk%2===0,blink=tk%11===0,lit=state!=='sleep';
  ...
  if(lit){const v=h%6;
    ...
    /* 벽 장식 */
    if(v%3===0)drawWindow(c,ix+RIW-32,iy+8,frame);else if(v%3===1)drawPoster(c,ix+RIW-26,iy+10);else drawBoard(c,ix+RIW-48,iy+10,frame);
    /* 가구 조합 */
    if(v===0){drawBed(c,ix+4,fy-34);drawPlant(c,ix+44,fy-24)}
    else if(v===1){drawSofa(c,ix+4,fy-30);drawPlant(c,ix+40,fy-24)}
    else if(v===2){drawServer(c,ix+6,fy-46,frame);drawBoxes(c,ix+28,fy-22)}
    else if(v===3){drawShelf(c,ix+8,iy+30);drawBoxes(c,ix+6,fy-22);drawPlant(c,ix+52,fy-24)}
    else if(v===4){drawShelf(c,ix+30,iy+24);drawPlant(c,ix+8,fy-24)}
    else{drawSofa(c,ix+4,fy-30);drawShelf(c,ix+42,iy+28)}
    if(h%4===2)drawPoster(c,ix+44,iy+28);
    drawDesk(c,ix+RIW-58,fy-26,state,frame);
    const shirt=SHIRTS[t.category]||SHIRTS.main;
    const poses={type:'type',panic:'panic',coffee:'sip',wait:'stand',sleep:'sleep'};
    const pose=poses[state];
    if(pose!=='stand'){px(c,ix+RIW-72,fy-26,10,3,'#5f4732');px(c,ix+RIW-70,fy-23,3,9,'#4a382a');px(c,ix+RIW-64,fy-23,3,9,'#4a382a')}
    const cx=pose==='wait'?ix+18:ix+RIW-74,cy=pose==='wait'?fy-40:fy-44;
