...
function renderStats(){const all=TASKS.tasks,act=all.filter(t=>!isDone(t)),today=act.filter(t=>dueState(t)==='today'||t.status==='doing'),over=act.filter(t=>dueState(t)==='overdue'||t.urgent),nodate=act.filter(t=>!t.due),doneToday=all.filter(t=>isDone(t)&&(t.completedAt||'').slice(0,10)===todayISO());document.getElementById('stats').innerHTML=`<span class="stat"><b>${today.length}</b>오늘</span><span class="stat ${over.length?'alert':''}"><b>${over.length}</b>지연·급함</span><span class="stat"><b>${nodate.length}</b>미정</span><span class="stat"><b>${doneToday.length}</b>오늘 완료</span>`}
