...
async function updateDates(id,due,endDate=''){const t=TASKS.tasks.find(x=>x.id===id);if(!t)return;const prev=[t.due,t.endDate];t.due=due;t.endDate=endDate;render();if(isDemo()){setStatus("데모","ok");return}try{await apiPost({action:'update_task',external_id:id,patch:{due_date:due||null,end_date:endDate||null}});setStatus('저장됨','ok');setTimeout(()=>setStatus('최신 상태','ok'),900)}catch(e){[t.due,t.endDate]=prev;showError('날짜를 저장하지 못했어요.');render()}}
